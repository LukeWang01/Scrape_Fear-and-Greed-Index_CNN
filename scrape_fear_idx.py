...
from _secret import LukeLab_Email, MY_EMAIL, RECEIVER_EMAILS, discord_channel_fgi_id
...
def get_time_index_list(hours=8, table_name='index_data'):
    ...
    end_msg = "```\n" + f"FGI Scraper Ended, from Road device, {len(time_index_list)} data points scraped. \n" \
    ...
    try:
        ...
        time.sleep(1)
        send_msg_to_discord_request(end_msg, channel_id=discord_channel_fgi_id)
    except Exception as e:
